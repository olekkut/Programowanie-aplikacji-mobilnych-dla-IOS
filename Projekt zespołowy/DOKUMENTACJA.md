...
* **Odporność na cenzurę:** Zdecentralizowana architektura platformy uniemożliwia przejęcie kontroli nad systemem lub jego wyłączenie przez jakikolwiek pojedynczy organ władzy. Dzięki rozproszeniu węzłów walidujących sieci Proof of Authority (PKW, Ministerstwo Cyfryzacji, NGO) oraz bezpośredniej integracji z rozproszonym ledgerem, referenda i ich wyniki są w docelowym systemie produkcyjnym całkowicie odporne na zewnętrzne manipulacje i blokady. **Uwaga: W jednowęzłowej wersji MVP cecha ta nie jest kryptograficznie zapewniona na poziomie sieci (szczegóły w Sekcji 5.3).**
...
* **Promowanie petycji [Planowane po-MVP]:** Narzędzie do oddolnego inicjowania spraw społecznych i lokalnych. Obywatele mogą tworzyć własne petycje, opisywać postulaty i zbierać cyfrowe, kryptograficznie zweryfikowane podpisy poparcia od innych mieszkańców. **Uwaga: Funkcjonalność ta została wyłączona z zakresu MVP (szczegóły w Sekcji 5.3).**
* **Kolektywne podejmowanie decyzji [Planowane po-MVP]:** Budowanie przestrzeni do deliberacji publicznej. Platforma umożliwia obywatelom udział w bezpiecznych dyskusjach nad projektami uchwał, zgłaszanie opinii i wspólne dochodzenie do konsensusu w sprawach referendalnych. **Uwaga: Funkcjonalność ta została wyłączona z zakresu MVP (szczegóły w Sekcji 5.3).**
...
2. **Ustawą o referendum lokalnym (Dz.U. 2000 nr 62 poz. 718):** Ustawa definiuje progi frekwencyjne (referendum jest ważne, jeżeli wzięło w nim udział co najmniej 30% uprawnionych do głosowania). System automatycznie zlicza frekwencję na podstawie liczby wydanych kryptograficznych tokenów autoryzacyjnych. **Uwaga: Gwarancja wiarygodności zliczania frekwencji zależy od braku możliwości fałszowania podpisów. W wersji MVP, ze względu na uproszczenie do pojedynczego serwera IdP, pełna gwarancja niepodatności na sztuczne generowanie frekwencji przez administratora jest ograniczona (szczegóły w Sekcji 6.2).**
...
> [!IMPORTANT]
> **Ograniczenie zakresu MVP:**
> Opisana poniżej lokalna weryfikacja tożsamości (NFC) oraz generowanie dowodów ZKP na urządzeniu mobilnym stanowią docelowy model bezpieczeństwa platformy. Ze względu na stopień skomplikowania kryptograficznego oraz brak dostępu do produkcyjnych certyfikatów rządowych (CSCA), funkcjonalność ta została **wyłączona z zakresu wersji MVP** i zastąpiona serwerowym Mock API (OAuth2 z logowaniem testowym PESEL, szczegóły w Sekcji 5.3).
...
sequenceDiagram
    ...
    Obywatel->>Chain: 9. Wysyła transakcję przez proxy relay (anonimizacja IP): [ID_Referendum, S, V, Sig]
...
> * **Anonimizacja sieciowa (IP):** Aplikacja mobilna wysyła transakcję do węzła blockchain za pośrednictwem proxy anonimizującego (Relay Proxy, patrz `TSK-14`), które usuwa adres IP klienta, zapobiegając ujawnieniu lokalizacji i tożsamości sieciowej głosującego.
...
1. **Obywatel (Aplikacja mobilna):**
   ...
   * Tworzenie i promowanie petycji obywatelskich oraz zbieranie cyfrowego poparcia od innych zweryfikowanych mieszkańców. **[Poza zakresem MVP]**
   * Udział w publicznych dyskusjach i kolektywnym podejmowaniu decyzji na dedykowanych forach konsultacyjnych powiązanych z referendami. **[Poza zakresem MVP]**
...
### 4.2. Szczegółowy User Flow Obywatela (Wersja Poprawiona)
1. **Ekran Główny:** Użytkownik widzi listę referendów odfiltrowaną dla jego regionu. Klika "Weź udział" przy wybranym referendum gminnym.
2. **Wybór opcji i przygotowanie głosu:** Obywatel zapoznaje się z pytaniem i na ekranie smartfona zaznacza swój wybór (np. "TAK"). Aplikacja lokalnie, bez łączenia z siecią, generuje losowy klucz seryjny ($S$), oblicza hash z połączenia numeru i oddanego głosu, a następnie "oślepia" go za pomocą losowego czynnika kryptograficznego.
3. **Weryfikacja Uprawnień:** Aplikacja przekierowuje użytkownika do Węzła Krajowego. Po udanym logowaniu serwer IdP (urząd) weryfikuje wiek i kod TERYT.
4. **Pobranie Ślepego Podpisu:** Aplikacja przesyła do serwera IdP wcześniej przygotowany "ślepy hash". Serwer sprawdza w bazie relacyjnej, czy dany obywatel jeszcze nie brał udziału w tym referendum. Jeśli nie, odnotowuje pobranie uprawnienia, podpisuje ślepy hash kluczem kryptograficznym referendum i odsyła podpis do użytkownika.
5. **Wysyłka Głosu:** Aplikacja lokalnie odślepia otrzymany podpis. Następnie przesyła zanonimizowaną paczkę (ID referendum, numer $S$, oddany głos oraz czysty podpis urzędu) do smart contractu na blockchainie, wykorzystując proxy anonimizujące IP.
6. **Potwierdzenie:** Użytkownik otrzymuje "Kwit referendalny" zawierający unikalny numer seryjny głosu ($S$) oraz ostateczny hash transakcji z blockchaina, co pozwala mu na późniejszą weryfikację.
...
* **Blockchain:** Jeden lokalny węzeł **Hyperledger Besu** działający w trybie deweloperskim PoA (z wdrożonym smart contractem w Solidity).
  ...
  > [!WARNING]
  > **Brak odporności na cenzurę w MVP:**
  > Użycie pojedynczego lokalnego węzła deweloperskiego oznacza całkowitą centralizację bazy danych w fazie MVP. Wszelkie gwarancje odporności na cenzurę i modyfikację historii bloków są nieaktywne w tej wersji, a ich celem w MVP jest wyłącznie walidacja funkcjonalna i testy integracji aplikacji mobilnej.
...
* **Atak Sybil (tworzenie wirtualnych tożsamości):** Smart contract akceptuje wyłącznie głosy podpisane poprawnym kluczem referendum. Klucz ten jest w posiadaniu urzędu (IdP), który podpisuje dane tylko po uwierzytelnieniu przez mObywatel. Osoba nieposiadająca realnego Profilu Zaufanego nie jest w stanie wygenerować poprawnego podpisu.
  ...
  > [!WARNING]
  > **Podatność złośliwego administratora w MVP:**
  > W architekturze MVP opartej o jeden serwer uwierzytelniania (IdP), złośliwy administrator systemu posiadający bezpośredni dostęp do bazy danych IdP oraz klucza prywatnego referendum może wygenerować poprawne ślepe podpisy (tokeny) dla nieaktywnych lub fikcyjnych numerów PESEL, oznaczając je w bazie jako "głosujące" i oddając w ich imieniu głosy.
  >
  > **Konsekwencja prawna w MVP:** Ponieważ administrator może w tej wersji potajemnie "głosować" za nieaktywnych obywateli, system MVP nie jest w stanie zagwarantować bezwzględnej kryptograficznej wiarygodności ustawowego progu frekwencyjnego 30%. Zliczanie frekwencji opiera się w tej fazie na założeniu pełnego zaufania do operatora Mock IdP.

  **Mitygacja i wykrywanie nadużyć:**
  1. **Audytowalność publiczna (Mechanizm Kwitów):** Łączna liczba oddanych głosów na blockchainie musi odpowiadać liczbie oznaczonych wyborców w relacyjnej bazie `VOTER_PARTICIPATION`. Jeśli administrator zagłosuje za obywatela, a ten spróbuje oddać głos lub sprawdzić status swojego uczestnictwa, fakt naruszenia zostanie natychmiast wykryty.
  2. **Docelowe rozwiązanie (Podpisy progowe - Threshold RSA):** Jak opisano w Sekcji 8, klucz prywatny urzędu zostaje podzielony na udziały rozdystrybuowane pomiędzy niezależne podmioty (np. PKW, Ministerstwo Cyfryzacji i NGO). Wygenerowanie podpisu wymaga kooperacji minimum 2 z 3 podmiotów, co eliminuje zagrożenie jednostronnego fałszerstwa przez administratora pojedynczego systemu.
...
> * **Priorytet:** 🟥 `HIGH` | **Zależność:** 🧠 `TSK-10`, 📜 `TSK-07`, 🌐 `TSK-14`
> * **Opis:** Integracja biblioteki `ethers.js` w React Native. Konfiguracja przesyłania parametrów głosu ([ID, S, Choice, Sig]) za pomocą transakcji do wdrożonego smart contractu za pośrednictwem serwera pośredniczącego celem ukrycia adresu IP.
...
> ### `TSK-14` 🌐 Konfiguracja serwera Proxy/Relay anonimizującego IP
> * **Priorytet:** 🟨 `MEDIUM` | **Zależność:** 🛠️ `TSK-02`
> * **Opis:** Implementacja lekkiego serwera pośredniczącego (Relay Proxy) w Express.js (w ramach backendu), który odbiera podpisane transakcje od aplikacji mobilnej i przekazuje je do węzła RPC Hyperledger Besu po usunięciu nagłówka z adresem IP klienta.
> * **Kryteria Akceptacji (DoD):** Węzeł RPC blockchaina rejestruje wyłącznie ruch sieciowy pochodzący z adresu IP serwera proxy (brak widoczności adresów IP użytkowników).
...
* **Sprint 3 (Integracja, Audyt i Testy):** Połączenie aplikacji mobilnej z blockchainem, integracja transakcji, uruchomienie Relay Proxy, moduł weryfikacji głosów (kwit) oraz testy bezpieczeństwa (`TSK-09`, `TSK-11`, `TSK-12`, `TSK-13`, `TSK-14`).
...
1. **Przejście na Dowody z Wiedzą Zerową (ZKP):** Wdrożenie pełnej lokalnej weryfikacji tożsamości przez odczyt NFC (e-Dowód) i generowanie dowodów ZKP (np. przy użyciu protokołu Semaphore) bezpośrednio na urządzeniu mobilnym. Wyeliminuje to całkowicie przesyłanie wrażliwych atrybutów do serwera.
2. **Kryptografia progowa (Threshold RSA):** Rozdzielenie klucza prywatnego urzędu do ślepych podpisów (Shamir's Secret Sharing) dystrybuowane między niezależne podmioty (np. PKW, Ministerstwo Cyfryzacji, NGO), aby uniemożliwić jednostronne fałszowanie podpisów przez administratora pojedynczego serwera IdP.
3. **Pełna Integracja z mObywatelem:** Podjęcie współpracy z Ministerstwem Cyfryzacji w celu wdrożenia oficjalnego produkcyjnego API mObywatela jako jedynego dostawcy tożsamości.
